...
const { dbConnect, dbDisconnect, } = require('../tests-utils/dbHandler.utils');
// let user
beforeAll(async () => {
    await dbConnect();
});
...
describe('Testing database', () => {
    test('Succesfully creates a new user', async () => {
        const pass = await bcryptjs_1.default.hash('12345', 10);
...
describe('Post endpoints', () => {
    ...
    test('Returns 404 for a signed in user with no accounts created', async () => {
        const logRes = await supertest_1.default(app_1.default).post('/user/login').send(userLogin);
        const res = await supertest_1.default(app_1.default)
            .get('/user/balances')
        ...
        await supertest_1.default(app_1.default).get(`/user/logout`).set('Cookie', `myCookie=${logRes.body.token}`);
    });
...
describe('Get Requests', () => {
    test('Returns status 301 for unauthenticated user', async () => {
        const res = await supertest_1.default(app_1.default).get('/user/balances');
    ...
    test('Returns 404 for invalid login details', async () => {
        const res = await supertest_1.default(app_1.default).post('/user/login').send(invalidUserLogin);
        expect(res.statusCode).toEqual(404);
    });
